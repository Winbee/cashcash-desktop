...
@Service()
@EntityRepository(CashSplit)
export default class CashSplitRepository extends Repository<CashSplit> {
    ...
    async findCustom(parameters: SplitParameters): Promise<CashSplit[]> {
        let qb = this.createQueryBuilder('s').orderBy('s.transactionDate', 'DESC');
